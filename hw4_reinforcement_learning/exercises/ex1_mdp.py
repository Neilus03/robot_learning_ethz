...
class PolicyIteration:
        ...
        while True:
            ...
            for s in range(self.env.n_states):
                ...
                for a in range(self.env.n_actions):
                    ...
                    # Compute the action-value under the *current* value function:
                    #
                    # Suggested steps:
                    ...
                    #   q_pi(s, a) = Σ_{s'} P(s'|s,a) [ r(s,a,s') + γ * V(s') ]
                    #
                    # For terminal transitions (done=True), the episode ends after receiving r,
                    # so there is no discounted continuation term.
                    for prob, next_state, reward, done in self.env.P[s][a]:
                        qsa += prob * (reward + self.gamma * self.v[next_state] * (1.0 - float(done)))

                    # Policy evaluation update:
                    #
                    #   V(s) = Σ_a π(a|s) q_pi(s,a)
            ...
            # Convergence check (Δ in the pseudocode).
            # Stop when the largest change across all states is below theta.
    ...
    def policy_improvement(self):
        ...
        for s in range(self.env.n_states):
            ...
            for a in range(self.env.n_actions):
                ...
                # Greedy improvement uses Q-values induced by the current V:
                #
    ...
    def policy_iteration(self):
        ...
        while True:
            ...
            # 1) Policy Evaluation Loop (until V converges for current π)
            self.policy_evaluation()

            # 2) Policy Improvement (greedy w.r.t. current V)
        ...
        while True:
            ...
            for s in range(self.env.n_states):
                ...
                for a in range(self.env.n_actions):
                    ...
                    # Value iteration update uses the Bellman optimality backup:
    ...
    def get_policy(self):
        ...
        for s in range(self.env.n_states):
            ...
            for a in range(self.env.n_actions):
                ...
                # Greedy policy from the converged V:
                #
                #   π(s) = argmax_a Q(s,a), with uniform tie-breaking
                for prob, next_state, reward, done in self.env.P[s][a]:
                    qsa += prob * (reward + self.gamma * self.v[next_state] * (1.0 - float(done)))
